...
export default function BrandInfo({ setActiveStep }: { setActiveStep: (step: number) => void }) {
    ...
    const formik = useFormik({
        ...
        onSubmit: async (values) => {
            ...
            // setActiveStep(4);
            setIsPending(true);
            const url = await getUrl(values.brandLogo);
            if (url.imageUrl) {
                mutate({
        ...
        <Box display={"flex"} flexDirection={"column"} gap={2} sx={{ width: '100%' }}>
            ...
            <Box display={"flex"} flexDirection={"column"} gap={2} px={{ sm: "7px", md: "20px" }}>
                ...
                <Card sx={{ bgcolor: "background.default", display: "flex", flexDirection: "column", gap: 1, boxShadow: "none" }}>
                    ...
                    <Box sx={{ display: "flex", width: { xs: "100%", sm: "90%", md: "80%" }, mx: "auto", flexDirection: { xs: "column", sm: "row", md: "row" }, justifyContent: "space-between", alignItems: "center", gap: 1 }}>
                        <Box display={"flex"} flexDirection={"column"} justifyContent={"center"} alignItems={"flex-start"} pb={{
                            xs: "0px", sm: "30px"
                        }} width={{ xs: "100%", sm: "fit-content" }}>
                            <GeneralInputField name="brandName" type="text" value={formik.values.brandName} onChange={(e) => formik.setFieldValue("brandName", e.target.value)} error={formik.touched.brandName && !!formik.errors.brandName} helperText={formik.errors.brandName} label="BRAND NAME" placeholder="Enter your brand name" disabled={false} />
                        </Box>
                        <Box flex={1} width={{ xs: "100%", sm: "fit-content" }} >
                            <SelectElement name="numberOfEmployees" value={formik.values.numberOfEmployees} onChange={(e) => formik.setFieldValue("numberOfEmployees", e.target.value)} error={formik.touched.numberOfEmployees && !!formik.errors.numberOfEmployees} helperText={formik.errors.numberOfEmployees} options={employeesNumberOptions} placeholder="Select your country" label="NUMBER OF EMPLOYEES" />
                        </Box>
                    </Box>
                ...
                <Card sx={{ bgcolor: "background.default", display: "flex", flexDirection: "column", gap: 1, boxShadow: "none" }}>
                    ...
                    >
                        <Grid size={{ xs: 12, md: 4 }}>
                            <Box pb={{ xs: "0px", sm: "27px" }}>
                                <GeneralInputField name="category" type="text" value={formik.values.category} onChange={(e) => formik.setFieldValue("category", e.target.value)} error={formik.touched.category && !!formik.errors.category} helperText={formik.errors.category} label="CATEGORY" placeholder="Enter your category" disabled={false} />
                            </Box>
                        </Grid>
                        <Grid size={{ xs: 12, md: 4 }}>
                            <SelectElement name="country" icon={<EarthIcon />} value={formik.values.country} onChange={(e) => formik.setFieldValue("country", e.target.value)} error={formik.touched.country && !!formik.errors.country} helperText={formik.errors.country} options={countriesOptions} placeholder="Select your country" label="COUNTRY" />
                        </Grid>
                        <Grid size={{ xs: 12, md: 4 }}>
                            <SelectElement name="city" value={formik.values.city} onChange={(e) => formik.setFieldValue("city", e.target.value)} error={formik.touched.city && !!formik.errors.city} helperText={formik.errors.city} options={citiesOptions} placeholder="Select your city" label="CITY" />
                        </Grid>
                        <Grid size={{ xs: 12, md: 4 }}>
                            <GeneralInputField name="district" type="text" value={formik.values.district} onChange={(e) => formik.setFieldValue("district", e.target.value)} error={formik.touched.district && !!formik.errors.district} helperText={formik.errors.district} label="DISTRICT" placeholder="Enter your district" disabled={false} />
                        </Grid>
                ...
                <Box display={"flex"} justifyContent={"center"} bgcolor={"red"} mx={"auto"} mt={"20px"} width={{ xs: "100%", sm: "70%", md: "50%" }}>
                    <GradientButton onClick={formik.handleSubmit} disabled={false} borderRadius="10px" width="100%" height="100%" >
                        {isPending ? <CircularProgress size={24} sx={{ color: "white", margin: "10px" }} /> :
                            <Typography
                                variant="h6"
                                fontWeight={500}
                                fontSize={16}
