...
import { IDIcon } from "@/app/components/icons/icons";
import Grid from "@mui/material/Grid";
import GeneralInputField from "@/app/components/inputFields/generalInputField";
import { useState } from "react";
import { LampIcon, CheckIcon } from "@/app/components/icons/icons";
import { useFormik } from "formik";
import ImageCardUploader from "@/app/components/inputFields/imageCardUploader";
...
export default function Verification({ setActiveStep }: { setActiveStep: (step: number) => void }) {
    ...
    return (
        <Box display={"flex"} flexDirection={"column"} gap={2} sx={{ width: "100%" }}>
            ...
            <Stack flexDirection={'column'} gap={2} p={{ sm: "8px", md: "16px" }} mt={"10px"}>
                {/* first card  */}
                {!idCard && <Card sx={{ bgcolor: "#F6F3EC", display: "flex", flexDirection: "column", gap: 1, padding: "32px", borderRadius: "8px" }}>
                    ...
                    <Stack spacing={2} flexDirection={"column"} width={"100%"}>
                        <GeneralInputField type="text" value={formik.values.fullName} onChange={(e) => formik.setFieldValue("fullName", e.target.value)} error={formik.touched.fullName && !!formik.errors.fullName} helperText={formik.errors.fullName} label="Full Name (As per ID)" placeholder="Enter your full name" disabled={false} />
                        <Box sx={{ display: "flex", flexDirection: { xs: "column", md: "row" }, gap: 1 }}>
                            <GeneralInputField type="text" value={formik.values.nationalId} onChange={(e) => formik.setFieldValue("nationalId", e.target.value)} error={formik.touched.nationalId && !!formik.errors.nationalId} helperText={formik.errors.nationalId} label="National ID Number" placeholder="Enter your national ID number" disabled={false} />
                            <GeneralInputField optional={true} type="text" value={formik.values.taxNumber} onChange={(e) => formik.setFieldValue("taxNumber", e.target.value)} error={formik.touched.taxNumber && !!formik.errors.taxNumber} helperText={formik.errors.taxNumber} label="Tax Number" placeholder="Enter your tax number" disabled={false} />
                        </Box>
                    </Stack>
            ...
            <Box display={"flex"} justifyContent={"center"} mt={"px"} sx={{ width: { xs: "100%", md: "50%" } }} mx={"auto"}>
                {idCard &&
